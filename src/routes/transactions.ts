...
// Listado con filtros básicos (periodo, categoría, estado) — alimenta el dashboard
...
// Creación manual desde el dashboard (Fase 4). Las creadas por correo/WhatsApp
// se generan desde los servicios internos (Fase 2 y 3), no por esta ruta pública.
...
// Confirmar o rechazar una transacción pendiente
// (esto es lo que dispara el botón "Sí, guárdalo" / "No, no lo guardes" en WhatsApp)
const decisionSchema = z.object({ decision: z.enum(["CONFIRMED", "REJECTED"]) });

transactionsRouter.patch("/:id/decision", async (req, res) => {
